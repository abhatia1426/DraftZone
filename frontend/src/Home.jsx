      ...
      >
        <div className="max-w-7xl mx-auto px-6 py-5 flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-8 text-sm font-bold uppercase tracking-wider">
            {['Home', 'Players', 'Draft', 'Odds', 'Login & Signup', 'Authors', 'Bets'].map((item) => (
              <Link
                key={item}
                to={item === 'Home' ? '/' : item === 'Players' ? '/player-search' : item === 'Draft' ? '/draft' : item === 'Odds' ? '/odds' : item === 'Login & Signup' ? '/login' : item === 'Authors' ? '/authors' : item === 'Bets' ? '/my-bets' : '#'}
