...
export default function PlayerDetail({ player, onBack }) {
  const { fadeUpSm } = useMotionPresets();
  ...
  return (
    <div>
      <button
        onClick={onBack}
        className={`flex items-center gap-2 text-sm mb-6 hover:opacity-70 transition-opacity rounded ${FOCUS_RING}`}
      ...
      <MotionDiv
        ...
        style={{ background: 'rgba(var(--bg-surface-rgb), 0.7)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: '1px solid rgba(var(--text-primary-rgb), 0.08)', boxShadow: '0 12px 32px rgba(var(--text-primary-rgb), 0.08)' }}
      >
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 mb-10">
            ...
            <div className="flex-1 text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
                ...
                <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>{player.team || 'FA'}</span>
              </div>
              <h1 className="dz-heading text-3xl font-bold" style={{ color: 'var(--text-primary)' }}>{player.full_name}</h1>
            </div>

            <div
              className="text-center rounded-2xl px-8 py-5"
              style={{ background: 'var(--accent-light)', border: '1px solid rgba(var(--accent-rgb), 0.15)' }}
            >
              <div className="text-4xl font-bold" style={{ color: 'var(--accent)' }}>
                {(player.stats?.pts_ppr || 0).toFixed(1)}
              </div>
              <div className="text-xs mt-1" style={{ color: 'var(--accent-text)' }}>2025 PPR pts</div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {statRows.map((stat) => (
                <div
                  ...
                  style={{ background: 'rgba(var(--text-primary-rgb), 0.03)', border: '1px solid rgba(var(--text-primary-rgb), 0.06)' }}
                >
      ...
      <MotionDiv
        ...
        style={{ background: 'rgba(var(--bg-surface-rgb), 0.7)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: '1px solid rgba(var(--text-primary-rgb), 0.08)', boxShadow: '0 12px 32px rgba(var(--text-primary-rgb), 0.08)' }}
      >
        <h2 className="dz-heading text-lg font-semibold mb-5" style={{ color: 'var(--text-primary)' }}>Season history</h2>

        {historyLoading ? (
          <div className="text-sm py-6 text-center" style={{ color: 'var(--text-muted)' }}>Loading past seasons…</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {history.map((season) => (
              <div
                ...
                style={{ background: 'rgba(var(--text-primary-rgb), 0.03)', border: '1px solid rgba(var(--text-primary-rgb), 0.06)' }}
              >
                <div className="text-xs font-semibold mb-2" style={{ color: 'var(--text-muted)' }}>{season.season}</div>
                <div className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>{season.pts_ppr.toFixed(1)}</div>
                <div className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>PPR points</div>
              </div>
            ))}
          </div>
        )}
      ...
      <MotionDiv
        ...
        style={{ background: 'rgba(var(--text-primary-rgb), 0.85)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.08)' }}
      >
        <div>
          <h2 className="dz-heading text-lg font-semibold mb-1" style={{ color: 'var(--text-inverse)' }}>Highlights</h2>
        ...
        <a
          ...
          className={`flex items-center gap-2 px-6 py-3 rounded-full text-sm font-medium hover:opacity-90 transition-opacity flex-shrink-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-bright)] focus-visible:ring-offset-2 focus-visible:ring-offset-[rgba(0,0,0,0.6)]`}
          style={{ background: 'var(--accent-bright)', color: '#0C2313' }}
        >
          <Youtube size={18} />
          Watch highlights
        </a>
